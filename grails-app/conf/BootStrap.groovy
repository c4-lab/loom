...
class BootStrap {
    ...
    private createInitialRecords() {
        def adminRole = Role.findWhere(authority: Roles.ROLE_ADMIN) ?: new Role(authority: Roles.ROLE_ADMIN).save(failOnError: true)
        def creatorRole = Role.findWhere(authority: Roles.ROLE_CREATOR) ?: new Role(authority: Roles.ROLE_CREATOR).save(failOnError: true)
